...
import webreport.tools.{KafkaSink, GetOracleDataframe}
...
object KafkaProducer {
  def main(args: Array[String]): Unit = {
    ...
    val tableName="(select * from dwr_pnl_hist where shift_timekey='20190702 060000') a"
    val pnl = GetOracleDataframe(spark,tableName)
    try{
      pnl.foreach(record=>{
